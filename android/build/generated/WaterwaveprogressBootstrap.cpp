...
#define TAG "ti.waterwaveprogress"

using namespace v8;

static Persistent<Object> bindingCache;

static void Waterwaveprogress_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Waterwaveprogress.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::WaterwaveprogressBindings::lookupGeneratedInit(
...
static void Waterwaveprogress_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Waterwaveprogress_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Waterwaveprogress_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::WaterwaveprogressBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry WaterwaveprogressBinding = {
	"ti.waterwaveprogress",
	Waterwaveprogress_init,
	Waterwaveprogress_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_ti_waterwaveprogress_WaterwaveprogressBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.waterwaveprogress", &WaterwaveprogressBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::WaterwaveprogressBindings::lookupGeneratedInit));
}
