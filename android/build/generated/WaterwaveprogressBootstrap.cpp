...
static Handle<Value> Waterwaveprogress_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Waterwaveprogress.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::WaterwaveprogressBindings::lookupGeneratedInit(
...
static void Waterwaveprogress_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Waterwaveprogress_getBinding)->GetFunction());
}

static void Waterwaveprogress_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::WaterwaveprogressBindings::lookupGeneratedInit(*binding, bindingLength);
	...
	(JNIEnv *env, jobject self)
{
	...
	titanium::KrollBindings::addExternalLookup(&(::WaterwaveprogressBindings::lookupGeneratedInit));
}
